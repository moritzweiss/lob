    ...
    for drift in ['down']:
        for env_type in ['simple']:
            ...
            if drift is None:    
                ...
                # r_submit_and_leave = np.load(f'data/rewards_{n_lots}_submit_and_leave_{env_type}.npy')
                # r_submit_and_leave_linear = np.load(f'data/rewards_{n_lots}_submit_and_leave_linear_{env_type}.npy')
                r_rl = np.load(f'data/rewards_{n_lots}_rl_{env_type}.npy') 
            else:
                r_all_passive = np.load(f'data/rewards_{n_lots}_all_passive_{env_type}_{drift}.npy') 
                r_linear_passive = np.load(f'data/rewards_{n_lots}_linear_passive_{env_type}_{drift}.npy')
                ...
                r_rl = np.load(f'data/rewards_{n_lots}_rl_{env_type}_{drift}.npy') 
            ...
            if drift is None:
                ...
                plt.savefig(f'plots/hist_{n_lots}_{name}_{drift}.pdf' )
                plt.savefig(f'plots/hist_{n_lots}_{name}_{drift}.png' )
            ...
            if drift is None:
                ...
                df.to_csv(f'tables/results_{n_lots}_{name}_{drift}.csv')
                df.to_latex(f'tables/results_{n_lots}_{name}_{drift}.tex', index=True, float_format="%.2f", bold_rows=True)
            # save to excel 
