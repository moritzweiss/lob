...
from limit_order_book.limit_order_book import LimitOrderBook, MarketOrder
...
class Market(gym.Env):
    def __init__(self, seed, n_steps=int(1e3), level=30, volume=10, side='ask', imbalance_reaction=False, linear_sl=False) -> None:
        self.imbalance_reaction = imbalance_reaction
        super().reset(seed=seed)
        self.n_steps = n_steps
        self.level = level
        self.noise_agent = NoiseAgent(level=level, rng=self.np_random, imbalance_reaction=self.imbalance_reaction, initial_shape_file='data_small_queue.npz', config_n=1)
...
class SampleMarket:
    def __init__(self, n_steps=int(1e3), n_samples=10, level=30, volume=10, side='ask', imbalance_reaction=False, place_at=1, initial_shape_file='data_small_queue.npz', linear_sl=False):
        self.level = level
        self.n_steps = n_steps
    ...
    def sample(self, seed):
        M = Market(seed=seed, imbalance_reaction=self.imbalance_reaction,  n_steps=self.n_steps, level=self.level, volume=self.volume, side=self.side, linear_sl=self.linear_sl)
        rewards = []        
    ...
    def mp_sample(self, n_workers=2,seed=1):
...
if __name__ == '__main__':
    ...
    volumes = [10, 20, 30, 40]
    # volumes = [20, 30]
    strategies = ['sl', 'linear_sl', 'market']
    ...
    for imbalance_reaction in reactions:
        for strategy in strategies:                   
            ...
            tag = f'{strategy}_{"without_imbalance" if not imbalance_reaction else "with_imbalance"}'
            data[tag] = []
            ...
            for volume in volumes:
                ...
                if strategy == 'sl':
                    ...
                    linear_sl = True    
                SM = SampleMarket(n_steps=max_steps, n_samples=n_samples, level=30, volume=volume, side='ask', imbalance_reaction=imbalance_reaction, linear_sl=linear_sl)
                if strategy == 'market':
                ...
                fig, ax = plt.subplots() 
                ax.hist(r, bins=np.arange(start=-5.5, stop=3.0, step=1.0), color='blue', align='mid') 
    ...
    data = data.round(2)
    data.to_latex(f'tables/rewards.tex', index=True, float_format="%.2f")
